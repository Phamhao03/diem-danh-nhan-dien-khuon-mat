...
class Teacher:
    def __init__(self,root):
        ...
        self.AttendanceReportTable.column("phone", width=120, anchor="center")
    ...
    def get_cursor(self, event=""):
        ...
        if rows:
            ...
            self.var_phone.set(str(rows[2]))  # Đảm bảo giữ số 0 đầu tiên
    ...
    def add_data(self):
        if self.var_id.get() == "" or self.var_name.get() == "":
            ...
            try:
                ...
                my_cursor = conn.cursor()
                my_cursor.execute(
                    "insert into teacher (Teacher_id, Name, Phone, Email, Password) values(%s,%s,%s,%s,%s)", (
                        self.var_id.get(),
                        self.var_name.get(),
                        self.var_phone.get(),
                        self.var_email.get(),
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(host='localhost', user='root', password='', database='face_recognizer',
                                       port='3306')
        my_cursor = conn.cursor()
        my_cursor.execute("SELECT Teacher_id, Name, CAST(Phone AS CHAR), Email, Password FROM teacher")
        ...
        if self.var_id.get() == "" or self.var_name.get() == "":
            ...
            try:
                ...
                if Update:
                    ...
                    my_cursor = conn.cursor()
                    my_cursor.execute("update teacher set Name=%s, Phone=%s, Email=%s, Password=%s where Teacher_id=%s",
                                      (
                                          self.var_name.get(),
                                          self.var_phone.get(),
                                          self.var_email.get(),
                    ...
                    self.reset_data()
                    conn.close()
                    messagebox.showinfo("Thành công", "Cập nhật thông tin giảng viên thành công", parent=self.root)
            except Exception as es:
        ...
        if self.var_com_search.get() == "" or self.var_search.get() == "":
            ...
            try:
                ...
                if self.var_com_search.get() == "ID GV":
                    search_column = "Teacher_id"
                elif self.var_com_search.get() == "Tên GV":
                    search_column = "Name"
                elif self.var_com_search.get() == "SĐT":
                    search_column = "Phone"

                my_cursor.execute(f"SELECT * FROM teacher WHERE {search_column} LIKE '%{self.var_search.get()}%'")
                data = my_cursor.fetchall()
                if data:
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
                    for i in data:
                        self.AttendanceReportTable.insert("", END, values=i)
                    messagebox.showinfo("Thông báo", f"Có {len(data)} bản ghi thỏa mãn điều kiện", parent=self.root)
                else:
                    self.AttendanceReportTable.delete(*self.AttendanceReportTable.get_children())
